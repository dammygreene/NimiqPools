...
const PORT = Number(process.env.LIMITS_TEST_PORT || 3231);
...
const POOL_CAP_LUNA = 250_000;
const STAKE_LUNA = 100_000;
const OVER_WALLET_CAP_LUNA = 200_000;
...
async function createPool(label, creatorAddress, stakeAmountLuna) {
  ...
  if (result.status !== 201) {
    throw new Error(`Pool creation failed for ${label}: ${JSON.stringify(result)}`);
  }
...
async function main() {
  ...
  try {
    ...
    const freshA = tempWallet();
    const freshB = tempWallet();
    const freshC = tempWallet();

    const fundA = await sendTx(client, rewards, freshA.address, 300_000, "LIMITS:FUND:A");
    const fundB = await sendTx(client, rewards, freshB.address, 300_000, "LIMITS:FUND:B");
    const fundC = await sendTx(client, rewards, freshC.address, 300_000, "LIMITS:FUND:C");
    await waitForConfirmations(client, fundA);
    await waitForConfirmations(client, fundB);
    await waitForConfirmations(client, fundC);

    const walletCapPool = await createPool("wallet-cap", escrow.address, OVER_WALLET_CAP_LUNA);
    const walletCapTx = await sendTx(client, freshA, escrow.address, OVER_WALLET_CAP_LUNA, `POOL:${walletCapPool.id}`);
    await waitForConfirmations(client, walletCapTx);
    const walletCapJoin = await api(`/api/pools/${walletCapPool.id}/join`, {
      method: "POST",
      body: JSON.stringify(predictionBody(freshA, walletCapPool, "Yes", walletCapTx)),
    });
    report.walletCap = {
      poolId: walletCapPool.id,
      txHash: walletCapTx,
      response: walletCapJoin,
    };

    const poolCapPool = await createPool("pool-cap", escrow.address, STAKE_LUNA);
    const stake1 = await sendTx(client, freshA, escrow.address, STAKE_LUNA, `POOL:${poolCapPool.id}`);
    const stake2 = await sendTx(client, freshB, escrow.address, STAKE_LUNA, `POOL:${poolCapPool.id}`);
    const stake3 = await sendTx(client, freshC, escrow.address, STAKE_LUNA, `POOL:${poolCapPool.id}`);
    await waitForConfirmations(client, stake1);
    await waitForConfirmations(client, stake2);
    await waitForConfirmations(client, stake3);
    const join1 = await api(`/api/pools/${poolCapPool.id}/join`, {
      method: "POST",
      body: JSON.stringify(predictionBody(freshA, poolCapPool, "Yes", stake1)),
    });
    const join2 = await api(`/api/pools/${poolCapPool.id}/join`, {
      method: "POST",
      body: JSON.stringify(predictionBody(freshB, poolCapPool, "No", stake2)),
    });
    const join3 = await api(`/api/pools/${poolCapPool.id}/join`, {
      method: "POST",
      body: JSON.stringify(predictionBody(freshC, poolCapPool, "Yes", stake3)),
    });
    report.poolCap = {
      poolId: poolCapPool.id,
      join1,
      join2,
      join3,
      txHashes: { stake1, stake2, stake3 },
    };

    const pauseBefore = await api("/api/admin/pause", {
      headers: { authorization: `Bearer ${TEST_ADMIN_TOKEN}` },
    });
    const setPaused = await api("/api/admin/pause", {
      method: "POST",
      headers: { authorization: `Bearer ${TEST_ADMIN_TOKEN}` },
      body: JSON.stringify({ paused: true }),
    });
    const pausedConfig = await api("/api/config");

    const pausedPool = await createPool("paused-join", escrow.address, STAKE_LUNA);
    const pausedTx = await sendTx(client, freshA, escrow.address, STAKE_LUNA, `POOL:${pausedPool.id}`);
    await waitForConfirmations(client, pausedTx);
    const pausedJoin = await api(`/api/pools/${pausedPool.id}/join`, {
      method: "POST",
      body: JSON.stringify(predictionBody(freshA, pausedPool, "Yes", pausedTx)),
    });
    const poolsRead = await api("/api/pools");
    const referralRead = await api(`/api/referrals?address=${encodeURIComponent(freshA.address)}`);
    const setUnpaused = await api("/api/admin/pause", {
      method: "POST",
      headers: { authorization: `Bearer ${TEST_ADMIN_TOKEN}` },
    ...
    report.pause = {
      ...
      setPaused,
      pausedConfig,
      pausedPoolId: pausedPool.id,
      pausedTxHash: pausedTx,
      pausedJoin,
      poolsReadStatus: poolsRead.status,
      referralReadStatus: referralRead.status,
      setUnpaused,
    };

    report.adminPauseState = await api("/api/admin/pause", {
      headers: { authorization: `Bearer ${TEST_ADMIN_TOKEN}` },
    });
  } catch (error) {
    report.error = {
